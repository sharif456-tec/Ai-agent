...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (location) {
      const redirected = safeTarget(new URL(location, target).toString());
      if (!redirected) return json({ detail: 'Unsafe redirect blocked' }, 400);
    }
    ...
    if (target.protocol !== 'https:') findings.push({ severity: 'high', title: 'Target is not using HTTPS', type: 'transport' });
    ...
    return json({ target: target.toString(), final_url: target.toString(), http_status: response.status, security_score: score, ...counts, findings, technologies, scanned_at: new Date().toISOString(), mode: 'passive-safe-audit-cloudflare' });
  } catch (error) {
